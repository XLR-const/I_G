...
NPC_CONFIG = {
    ...
    '6': {
        ...
        '7': {
        'name': 'AutoGunGuy',          # Должно строго совпадать с именем папки со спрайтами!
        'speed': 0.28,                 # Скорость бега (чуть быстрее обычного солдата)
        'hp': 120,                     # Здоровье (крепкий парень, выдержит пару попаданий)
        'damage': 6,                   # Урон за одну пулю. Так как он бьет очередью по 3 пули, 
                                       # суммарно за один залп он снимет около 18 HP!
        'shoot_range': 7.0,            # Дальность ведения огня (начинает стрелять издалека)
        'shoot_delay': 1400,           # Кулдаун в мс между его тройными очередями
        'sound_volume': 0.25,          # Громкость его автомата
    },
