...
<!-- 🔥 ДЕМОНСТРАЦИЯ ГЕЙМПЛЕЯ И СИСТЕМ (АНИМИРОВАННАЯ ГИФКА) -->
...
* **Графический подслой**: Pygame `==2.6.1` (Управление окном, поверхностями, пиксельными буферами и событиями ввода).
* **Вычислительный модуль**: NumPy `==2.4.6` (Высокоскоростные многомерные матрицы карт и векторов движения).
* **JIT-Компилятор оптимизации**: Numba `==0.66.0` + llvmlite `==0.48.0` (Компиляция тяжелых пиксельных циклов рейкастинга «на лету» в машинный код Си-уровня через LLVM).
* **Инструментарий медиа и анимаций**: opencv-python `==4.8.1.78`, pillow `==12.3.0`, ffmpeg-python `==0.2.0`, vidmaker `==2.3.0` (Пайплайн видеофрагментов, катсцен, генерации анимаций смерти и текстур).
* **Сборщик релиза и документации**: pyinstaller `==6.21.0`, reportlab `==5.0.0` (Компиляция дистрибутива в папку и генерация PDF-отчетов).
...
3. **Установка строго зафиксированных версий библиотек:**
```bash
