...
class Weapon:
    ...
    def _load_sprite(self):
        ...
        config = WEAPON_CONFIG.get(self.weapon_name, {})
        sprite_path = config.get('sprite', f'resources/weapons/{self.weapon_name}.png')
        try:
    ...
    def _load_sound(self):
        """Загружает звук выстрела"""
        config = WEAPON_CONFIG.get(self.weapon_name, {})
        sound_path = config.get('sound', f'resources/weapons/{self.weapon_name}_shot.wav')
        try:
    ...
    def draw(self):
        """Рисует оружие (заглушка, переопределяется в дочерних)"""
        pass
