...
class LevelGenerator:
    ...
    def generate_and_save(self, level_num, seed, style, num_rooms):
        ...
        # =============================================================
        # 🔥 ШАГ 1. ТЕКСТУРНЫЙ ПАСПОРТ СТЕН (Уходим от голых единичек)
        # Привязываем уникальные символы стен из твоего DECOR_CONFIG / SYMBOLS_CONFIG
        # =============================================================
        style_walls = {
            'lab': '1',   # Белая научная панель
            'vent': '2',  # Ржавый металл вентиляции
            'hall': '3',  # Мраморные стены / Колонны
            'out': '4'    # Внешняя обшивка базы / Скалы
        }
        wall_char = style_walls.get(style, '1')

        # Заливаем карту монолитными базовыми стенами выбранного стиля
        grid = [[wall_char for _ in range(self.width)] for _ in range(self.height)]
        rooms_y_centers = []
        start_y = 4
        end_y = self.height - 5
        ...
        # =============================================================
        # ГЕНЕРАЦИЯ СТРУКТУРЫ ПО СТИЛЯМ (С динамической подменой текстур)
        # =============================================================
        if style == "lab":
            ...
            # Возводим бункеры из стен нужного уличного типа ('4')
            for i in range(num_rooms):
                t = i / max(1, num_rooms - 1) if num_rooms > 1 else 0.5
                ...
                for y in range(by, by + bh):
                    for x in range(bx, bx + bw):
                        if 1 <= x < self.width - 1 and 1 <= y < self.height - 1:
                            grid[y][x] = wall_char # Ставим стену уличного типа
                
                if bw > 3 and bh > 3:
                    for y in range(by + 1, by + bh - 1):
                        for x in range(bx + 1, bx + bw - 1):
                            grid[y][x] = "_"
                    grid[by + bh - 1][bx + bw // 2] = "_" # Дверь бункера
                else:
                    grid[by][bx] = "_"
            ...
            # =============================================================
            # 🔥 ШАГ 2. ИСПРАВЛЕНИЕ КРИВЫХ КОРИДОРОВ ДУГ (Широкие залы для холла)
            # =============================================================
            for i in range(len(rooms_y_centers) - 1):
                ...
                for y in range(s_y, e_y + 1):
                    ...
                    if style == "hall":
                        ...
                        # 🔥 РАСШИРЯЕМ ПРОХОД ДЛЯ ХОЛЛА: Вырезаем проход шириной в 3 клетки!
                        # Больше никаких узких кривых щелей в огромных мраморных залах!
                        for offset_x in (-1, 0, 1):
                            if 1 <= center_x + offset_x < self.width - 1:
                                grid[y][center_x + offset_x] = "_"
                    else: # vent
        ...
        # =============================================================
        # 🔥 ШАГ 3. ПРОЦЕДУРНОЕ ЗАСЕЛЕНИЕ И КЭШИРОВАНИЕ ПУСТЫХ КЛЕТОК
        # Находим все пустые зоны "_" и безопасно расставляем NPC и лут
        # =============================================================
        empty_cells = []
        for r in range(self.height):
            for c in range(self.width):
                if grid[r][c] == "_":
                    # Собираем пул пустых ячеек, отступая от краев старта и финиша
                    if r > 5 and r < (self.height - 6):
                        empty_cells.append((r, c))

        # Перемешиваем пул клеток для случайного распределения
        random.shuffle(empty_cells)

        # Рассчитываем количество спавна от размера пустых зон
        num_enemies = int(len(empty_cells) * 0.04) # 4% клеток займут монстры
        num_loot = int(len(empty_cells) * 0.06)    # 6% клеток займут патроны/аптечки

        # Спавним NPC под атмосферу стилей
        for _ in range(num_enemies):
            if not empty_cells: break
            r, c = empty_cells.pop()
            
            # Подставляй сюда СИМВОЛЫ монстров из своего SYMBOLS_CONFIG!
            if style == "lab":
                grid[r][c] = "Z"  # Зомби-ученый
            elif style == "vent":
                grid[r][c] = "I"  # Имп в туннеле вентиляции
            elif style == "hall":
                grid[r][c] = "K"  # Тяжелый штурмовик в залах
            else:
                grid[r][c] = "M"  # Рядовой NPC / монстр для улицы

        # Спавним предметы (лут)
        for _ in range(num_loot):
            if not empty_cells: break
            r, c = empty_cells.pop()
            
            # Подставляй сюда СИМВОЛЫ предметов из своего SYMBOLS_CONFIG!
            # 'A' - патроны (Ammo), 'H' - аптечка (Health)
            grid[r][c] = random.choice(["A", "H"])

        # =============================================================
        # 📐 РАССТАНОВКА СТАРТА И ВЫХОДА (ИСПРАВЛЕНО ДЛЯ ДВУМЕРНОГО МАССИВА)
        # =============================================================
        if style in ["lab", "out"]:
            ...
            if rooms_y_centers:
                # Берем координаты центра первой и последней комнаты из нашего массива кортежей
        ...
        # =============================================================
        # 💾 ТВОЙ РОДНОЙ ИСПРАВЛЕННЫЙ БЛОК СОХРАНЕНИЯ В JSON (БЕЗ КРАШЕЙ)
        # =============================================================
        # Настройки палитры освещения в зависимости от номера уровня
        ...
        # Формируем метаданные инвентаря и ассетов
        meta_data = {
        ...
        # Склеиваем карту и метаданные в красивый валидный JSON-пакет
        meta_json_string = json.dumps(meta_data, indent=2, ensure_ascii=False)
        final_json_content = "{\n" + f'  "map": {map_json_string},\n' + meta_json_string[2:]
        
        # Создаем директорию и записываем файл на диск
